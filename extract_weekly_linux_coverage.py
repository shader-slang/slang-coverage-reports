...
def main():
    ...
    # Group by week (taking the latest data point in each week)
    weekly_data = defaultdict(lambda: {'date': None, 'data': None})

    for date, data in sorted(daily_data.items()):
        week_start = get_week_start(date)
        # Always use the latest data in the week
        if weekly_data[week_start]['date'] is None or date > weekly_data[week_start]['date']:
            weekly_data[week_start]['date'] = date
            weekly_data[week_start]['data'] = data
